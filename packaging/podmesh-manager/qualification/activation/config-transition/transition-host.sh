#!/bin/bash
# Bounded and resumable zero-grant to three-grant local configuration transition.
set -euo pipefail
export LC_ALL=C
umask 077
...
usage() {
  echo "Usage: $0 --mode apply|rollback|seal-for-activation --host-alias lab-a|lab-b|lab-c --mapping <private-map.json> --salt-file <private-salt> --candidate-verification <report.json> --backup-file </root/file> --evidence-directory <private-directory>" >&2
  exit 2
}
mode= alias_name= mapping= salt= report= backup= evidence=
while [ "$#" -gt 0 ]; do
  case "$1" in
    --mode) mode=${2-}; shift 2;;
...
assert_still_quiescent() {
  ...
  [ -z "$(find -P "$state" -mindepth 1 -print -quit)" ] && [ ! -e "$socket" ] && [ ! -L "$socket" ] || { echo 'Manager state or control socket appeared during transition' >&2; exit 1; }
...
write_ledger() {
  ...
  rm -f -- "$tmp"
  python3 - "$tmp" "$state_name" "$alias_name" "$original_hash" "$applied_hash" "$backup_hash" "$mapping_hash" "$report_hash" "$package_version" "$binary_hash" "$markers" <<'PY'
import json, os, sys
path, state, alias, original, applied, backup, mapping, report, version, binary, markers = sys.argv[1:]
with open(path, "x", encoding="utf-8") as stream:
    json.dump({"schema_version":"podmesh-manager-config-transition-ledger/v1","state":state,"host_alias":alias,"original_sha256":original,"applied_sha256":applied,"backup_sha256":backup,"mapping_sha256":mapping,"candidate_report_sha256":report,"package_version":version,"binary_sha256":binary,"activation_markers_sha256":markers,"rollback_window_open":state in {"prepared","applied"}}, stream, sort_keys=True, indent=2)
...
read_ledger() {
  ...
  python3 - "$ledger" "$alias_name" "$mapping_hash" "$report_hash" "$package_version" "$binary_hash" <<'PY'
import json, sys
...
write_result() {
  ...
  python3 - "$tmp" "$action" "$alias_name" "$(commit_text original-config "$original")" "$(commit_text applied-config "$applied")" "$(commit_text protected-backup "$backup_sum")" "$(commit_file alias-map "$mapping")" "$(commit_file candidate-report "$report")" "$package_version" "$binary_hash" "$(commit_text activation-markers "$markers")" "$shape" "$grants" <<'PY'
import json, os, sys
path, action, alias, original, applied, backup, mapping, report, version, binary, markers, shape_path, grants_json = sys.argv[1:]
...
if [ "$mode" = apply ]; then
  if [ ! -e "$ledger" ] && [ ! -L "$ledger" ]; then
    ...
    python3 "$root/config-tool.py" --mode prepare --original "$config" --mapping "$mapping" --host-alias "$alias_name" --output "$candidate"
    ...
    python3 "$root/config-tool.py" --mode prepare --original "$backup" --mapping "$mapping" --host-alias "$alias_name" --output "$candidate"
  ...
  python3 "$root/config-tool.py" --mode verify --original "$backup" --candidate "$config" --mapping "$mapping" --host-alias "$alias_name" > "$shape"
  assert_still_quiescent
...
python3 "$root/config-tool.py" --mode verify --original "$backup" --candidate "$candidate" --mapping "$mapping" --host-alias "$alias_name" > "$shape"
current_hash=$(hash_file "$config")
if [ "$mode" = seal-for-activation ]; then
  ...
  rm -f -- "$seal_tmp"
  python3 - "$seal_tmp" "$alias_name" "$(commit_file candidate-report "$report")" "$(commit_text activation-markers "$markers")" <<'PY'
import json, os, sys
path, alias, candidate, markers = sys.argv[1:]
with open(path,"x",encoding="utf-8") as stream:
    json.dump({"schema_version":"podmesh-manager-rollback-window-closure/v1","result":"PASS","host_alias":alias,"rollback_window":"permanently-closed-for-this-transition","candidate_report_commitment":candidate,"activation_markers_commitment":markers,"state_directory_empty_at_closure":True,"manager_inactive_at_closure":True,"private_values":"absent"},stream,sort_keys=True,indent=2)
    stream.write("\n"); stream.flush(); os.fsync(stream.fileno())
PY
