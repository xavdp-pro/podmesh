...
try:
    ...
    gate_ready()
    tool('rotate', '--universe', LOGICAL, '--host', os.environ['PODMESH_SOURCE_SSH'], '--lease', '300', '--margin', '5')
    ...
    r = pub('publisher_start', previous={'none': True})
    ...
    daemon()
    assert pub('publisher_start', previous={'none': True}).get('ok')
    code, body = origin()
