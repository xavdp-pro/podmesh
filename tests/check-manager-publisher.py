...
try:
    ...
    for other in aliases[1:]:
        hosts[other].ok(request('activation_require', LOGICAL, reference, lease_seconds=120, takeover_margin_seconds=5, desired_standbys=len(aliases) - 1, authority_id=rot['permit']['authority_id']))
    e1 = rot['epoch']
    for a in aliases:
    ...
    r = pub('publisher_start', G, previous={'none': True})
    assert r.get('ok'), r
    st = wait_connector(G)
    ...
    fence = hosts[G].ok({'operation': 'activation_fence', 'operation_id': str(uuid.uuid4()), 'authorization_ref': reference, 'timeout_seconds': 10})
    ...
    r = pub('publisher_start', S, previous={'fenced': True, 'operation_id': fence.get('operation_id', 'fence')})
    assert r.get('ok'), r
    st = wait_connector(S)
