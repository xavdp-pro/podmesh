...
then proves the apply acts only with the stated fresh proofs and refuses every exclusion.
    ...
    PODMESH_SOCKET=... PODMESH_STATE_DIR=... PODMESH_UNIT=... python3 -B tests/check-migration-collector.py

The earlier lots' rows are read-only planning input: the plan enumerates them and reports why they are
blocked, and no apply of this suite ever names one. Every product mutation goes through the API. Direct
...
def fixture(host, name, *args):
    ...
    host.call('podman_run', args=['create', '--pull=never', '--network=none', '--name', name, *args])
...
try:
    ...
    assert any('still open (state issued)' in b for c in blocked for b in c['blockers']), blocked[:2]
    assert survey['settled_reservations']['count'] > 0 and all(
    ...
    # ------------------------------- a run interrupted after its effect and before its record recovers it
    # The effect is written with the run's own progress row in one transaction, so a service that dies
    # before it can report comes back to what it did. The interruption is simulated by removing exactly what
    # such a crash loses: the run record and the operation's persisted result.
    R = str(uuid.uuid4())
    ...
    interrupted = collection('garbage_collect_apply', plan_operation_id=r_plan['collection_operation_id'],
                             candidates=[{'class': CLASS2, 'universe_uuid': R}])
    once = A.ok(interrupted)
    assert once['effects_applied'] == 1 and once['effects_recovered'] == 0 and once['results'][0]['verified'] is True
    A.call('journal_delete', table='garbage_collection_runs', key_column='operation_id', key=interrupted['operation_id'])
    A.call('journal_write', table='operations', key_column='id', key=interrupted['operation_id'],
           values={'status': 'pending', 'result': None})
    before_recovery = A.call('snapshot')
    recovered = A.ok(dict(interrupted),
                     'a run whose record was lost after its effect recovers that effect from the journal instead of repeating it', checks)
    ...
    assert A.call('gc_runs')['count'] > 0 and A.call('journal_row', table='garbage_collection_runs',
                                                     key_column='operation_id', key=interrupted['operation_id'])['rows']
    checks.append('after the recovery the journal, the response and the observed state agree: one collection of that universe, one tombstone, '
                  'one run record rewritten, and no second effect')
    results['recovery'] = {'first': once['results'][0]['verified'], 'second': recovered['results'][0]}
    ...
    watched = B.call('restore_under_watchdog', request=runaway, floor_bytes=WATCHDOG_FLOOR, graph=GRAPH, timeout=1800)
    ...
    reported = refusal['details']['detail']['detail']['runtime_processes']
    ...
    assert budgeted['runtime_reclaims_performed'] == 1 and budgeted['processes_signalled'] > 0
    ...
    checks.append('[destination] verified from outside the collector: %d processes signalled on proof, both cgroup directories gone, the '
                  'container absent, the claim closed, and the graph root back from %d to %d MiB free'
                  % (budgeted['processes_signalled'], watched['minimum_free'] // 1024 ** 2, free_after // 1024 ** 2))
