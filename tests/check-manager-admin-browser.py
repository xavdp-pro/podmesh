#!/usr/bin/env python3
"""The administration page driven by a real headless browser (tests/browser-manager-admin.mjs),
against the responder, a stub resident holding the deployment account with its default password
and the must-change flag, and a stub control socket. What a server-side test cannot see is
...
server = subprocess.Popen([sys.executable, '-B', str(ORIGIN)], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
try:
    for _ in range(50):
        try:
