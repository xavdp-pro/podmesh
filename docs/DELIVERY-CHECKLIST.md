...
- [ ] Test removal/reinstallation and documented identity retention.
- [ ] Test interrupted operations and clone-of-host identity rejection. Only an interrupted clone commit is covered (see Lifecycle); other interruptions and clone-of-host identity rejection remain untested.
...
- [ ] Add explicit start/stop with operation evidence.
...
- [ ] Integrate and validate migration through the service, including return journeys, interrupted transfer/restore and recovery. Existing standalone kit evidence does not validate the service API.
