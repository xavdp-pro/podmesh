...
Implemented on 2026-09-12 by Claude Code (Opus 5) in the development tree and exercised on two lab hosts.
The binding document for this lot is [GARBAGE-COLLECTION.md](GARBAGE-COLLECTION.md), which the lot did not
edit: anything it should say differently was raised as a finding in the lot's report. Four of those findings
were accepted on 2026-09-13 by the operator and OpenAI Codex and the contract now carries them
(`evidence/garbage-collection/independent-review-codex-2.md`); the entries below say, for each, whether this
lot's code implements the amended wording or only reports the shape it names. Each entry is either a
decision the contract left open or a difference from what it or this protocol describes. Xavier may amend
...
this lot assumes they exist. Neither hold scope the contract now defines — `evidence_hold` and
`investigation_hold` — is implemented here, and this lot claims neither behaviour.
...
17. **A gap the classes leave, reported and not widened — the amendment was since accepted, and is still
    not implemented.** A reservation whose authorizations all ended `not_restored` **and** whose container
    is gone matches neither class 1, which requires a fresh observation of the stopped, checkpointed source
    it releases, nor class 2, which required that no authorization was ever issued. The collector reports it
    as blocked. On 2026-09-13 the operator and OpenAI Codex accepted the proposed widening, and
    [GARBAGE-COLLECTION.md](GARBAGE-COLLECTION.md) now requires that **no authorization is live** — none
    ever issued, or every one terminal with a verified `not_restored` outcome bound to its authorization,
    handoff, universe, source and destination — with the container-absent condition still mandatory. **This
    lot does not implement the widened class 2**: the code still requires that none was ever issued, and the
    shape above is still reported as blocked. Closing the gap is a later lot with its own tests.
