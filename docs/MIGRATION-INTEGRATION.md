...
Status: partially implemented. The source preparation and checkpoint, the transfer authorization, the destination preflight and restore, the completion and the source retirement are packaged since `0.1.0~experimental5` and were exercised forward and back between two lab hosts for one workload shape. Reservation release, abandonment and local restore exist since lot M3, with the failed-restore cleanup its review asked for; a garbage collector that plans on proof and applies within explicit bounds exists since lot M4, for terminal reservations and failed restore claims only, in the development tree and **not packaged**; placement commit is not implemented. LOCAL-API.md, MIGRATION-PROTOCOL.md and GARBAGE-COLLECTION.md describe what exists and record every deviation; this page keeps the boundaries, the evidence scope and the remaining gaps.
...
## Implemented source phase (packaged since 0.1.0~experimental5)
...
- **Release/recovery:** implemented for a reservation that never left this host. PodMesh does record transfer authorizations, so it can prove their absence for a given reservation: release and abandonment are refused once one was issued, and only a verified destination outcome ends the reservation then. Local recovery is an explicit choice between `migration_restore_local`, which resumes the checkpointed memory, and an ordinary `start`, whose result states that memory was not restored. A reservation whose authorizations **all** ended `not_restored` is now a garbage collection candidate (class 1) rather than a dead end: the collector proves each outcome still binds to its authorization, observes the source stopped and checkpointed, and records a terminal `collected` state with a tombstone that lifts the generic gate while refusing a blind `create` of that universe UUID for good. What remains open: there is still no break-glass path when a destination is unreachable or its outcome cannot bind — a transport that alters the documents still leaves the source authorized, and the collector refuses it too — and a reservation whose authorizations ended `not_restored` while its container is *also* gone matches neither collection class.
- **Garbage collection:** implemented for terminal reservations (contract classes 1 and 2, with tombstones) and for failed restore claims (class 3), the latter by calling `migration_restore_abort` rather than duplicating any process management. The read-only plan is also the first host-wide listing of what a host still owes a decision on. Not implemented: a failed **local** restore (class 4) and operation artifacts after a declared retention, with evidence holds and retained manifests (class 5) — so nothing in this version ever deletes an artifact. No timer, no monitoring agent, and no production mandate: the collector plans, and a named operator decision applies.
