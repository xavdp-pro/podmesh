...
| G2 Authenticated exchange | Three real processes, bounded mutual peer authentication and durable imports | Not yet qualified. Two live three-host campaigns of candidate `0.1.0~manager2+gff77b1f946e8` each converged to one canonical digest on all three replicas, at the converged stage and again after typed cleanup, with unchanged unrelated services, containers, routes and firewall. The checked-in comparator still does not PASS: every store retains outbound exchange attempts the candidate deliberately leaves uncertain when a peer at its one-connection incoming limit drops the connection, and the gate requires zero. Those attempts are permanent — the audit table is append-only by trigger — so a further campaign on the same stores inherits them and no configuration change alone can reach the gate. An operator decision on the durable stores is required first; see [REVIEW-MANAGER2-G2-LIVE-ACTIVATION.md](REVIEW-MANAGER2-G2-LIVE-ACTIVATION.md) |
