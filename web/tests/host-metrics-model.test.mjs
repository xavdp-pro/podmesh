import test from 'node:test';import assert from 'node:assert/strict';import {formatBytes,formatCpuCores,meshMemory} from '../src/hostMetricsModel.mjs';
...
test('CPU core counts are rendered as counts rather than bytes',()=>{assert.equal(formatCpuCores(2),'2');assert.equal(formatCpuCores(.25),'0.25');assert.equal(formatCpuCores(null),'Unknown');});
