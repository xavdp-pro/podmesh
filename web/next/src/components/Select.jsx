...
export default function Select({ value, onChange, options, placeholder = '—', className = '', searchable = false, searchPlaceholder = '…', noResult = '—', clearSearchLabel = 'Clear the search', label, disabled = false }) {
  ...
  const toggle = () => { if (disabled) return; setOpen(v => { if (!v) { setFocused(selectedIndex >= 0 ? selectedIndex : 0); if (canSearch) setQuery('') } return !v }) }
  ...
  return (
    <div ref={box} className={`relative ${className}`}>
      <button type="button" onClick={toggle} onKeyDown={onKeyDown} aria-haspopup="listbox" aria-expanded={open} aria-label={label} disabled={disabled}
        className="flex w-full items-center justify-between gap-2 rounded-md border border-cream-400 bg-cream-50 px-3 py-2 text-left text-sm transition-colors focus:border-green-600 focus:outline-none focus:ring-2 focus:ring-green-600/20 disabled:opacity-60">
          ...
          {canSearch && (
            <div className="border-b border-cream-300 bg-cream-100 px-3 py-2">
              <div className="relative">
                <input value={query} onChange={e => { setQuery(e.target.value); setFocused(0) }} onKeyDown={onKeyDown} placeholder={searchPlaceholder} aria-label={searchPlaceholder} autoFocus
          ...
          <ul ref={list} role="listbox" className="max-h-52 overflow-auto py-1">
            {visible.length === 0 ? <li className="px-3 py-2 text-sm text-ink-500">{noResult}</li> : visible.map((o, i) => (
              <li key={o.value} role="option" aria-selected={o.value === value} onMouseEnter={() => setFocused(i)} onMouseDown={() => pick(o)}
                className={`cursor-pointer select-none px-3 py-2 text-sm transition-colors ${i === focused ? 'bg-green-100 text-green-700' : o.value === value ? 'font-medium text-green-600' : 'text-ink-700 hover:bg-cream-200'}`}>
                {o.label}{o.hint && <span className="ml-2 text-xs text-ink-500">{o.hint}</span>}
              </li>
            ))}
          </ul>
        </div>
