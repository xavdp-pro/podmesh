...
pub fn open_state(dir: &Path) -> Result<Connection, Box<dyn std::error::Error>> {
    ...
    tx.execute(
        "INSERT OR IGNORE INTO metadata VALUES('machine_id',?1)",
        [&machine],
    )?;
    let uuid = fs::read_to_string("/proc/sys/kernel/random/uuid")?;
    tx.execute(
        "INSERT OR IGNORE INTO metadata VALUES('host_uuid',?1)",
        [uuid.trim()],
    )?;
    tx.commit()?;
        ...
        Ok(match op {
            ...
            "capabilities" => json!({
                ...
                "experimental_contracts":{
                    ...
                    "migration_restore_abort":"destination: never removes a running or verified universe; removes only a non-running container created by a held claim, or declines an unclaimed authorization, then records not_restored and writes the outcome. With the explicit reclaim_processes: true it first ends the processes it can prove belong to the failed attempt (a pidfd opened before membership/start-time revalidation is the only signal transport, so numeric PID reuse cannot redirect SIGKILL) and verifies that both cgroups disappeared; without it, surviving processes are reported and nothing is removed or signalled",
