...
pub fn handle(db: &Connection, request: &Value) -> Value {
    ...
    let result: Result<Value, Box<dyn std::error::Error>> = (|| {
        Ok(match op {
            ...
            "recovery_point_prepare" | "recovery_point_status" => recovery_point::execute(db, request)?,
            "migration_status" => migration::status(db, request)?,
            "capabilities" => json!({
                "version":option_env!("PODMESH_PACKAGE_VERSION").unwrap_or(env!("CARGO_PKG_VERSION")),
                "operations":["capabilities","identity","inventory","observations","activation_require","activation_acquire","activation_renew","activation_release","activation_status","activation_fence","recovery_point_prepare","recovery_point_status","create","delete","clone","start","stop"],
